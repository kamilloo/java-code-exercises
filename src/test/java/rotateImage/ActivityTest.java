...
public class ActivityTest {
    ...
    public void rotate_TwoDimensionsCubic(){
            ...
            int[][] expected = new int[][]{ new int[]{3,1}, new int[]{4,2}};

            Activity activity = new Activity();

        int[][] rotatedCubic = activity.rotate(cubic);

        Assertions.assertEquals(expected[0][0], rotatedCubic[0][0]);
        Assertions.assertEquals(expected[0][1], rotatedCubic[0][1]);
        Assertions.assertEquals(expected[1][0], rotatedCubic[1][0]);
        Assertions.assertEquals(expected[1][1], rotatedCubic[1][1]);
    }
